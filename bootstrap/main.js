    ...
    var table = $('#mytable').DataTable({
        ...
        "columns": [
            ...
            { "data": "state" },
            {
                "mData": "action",
                "mRender": function (data, type, full) {
                    return "<button class='btn-primary me-2 action-btn' id='" + data + "' ><a href='index.php?id=" + data + "'>Update</a></button><button class='btn-danger action-btn' id='" + data + "' onclick='confirmDelete(" + data + ")'>Delete</button>";
                }
            }
        ]
    });
...
$("#form").submit(function (event) {
    ...
    if ($("#password").is(":disabled")) {
        console.log("passwords disabled");
        passwordDisabled = 1;
    }
